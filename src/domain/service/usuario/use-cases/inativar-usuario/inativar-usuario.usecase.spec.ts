...
import { InativarUsuarioUseCase } from './inativar-usuario.usecase';
...
describe('InativarUsuarioUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    inativarUsuarioUseCase = moduleRef.get<InativarUsuarioUseCase>(
      InativarUsuarioUseCase,
    );
  ...
  it('should be defined', () => {
    expect(inativarUsuarioUseCase).toBeDefined();
  ...
  describe('execute', () => {
    it('should throw User NotFoundException if user is inactive or does not exist', async () => {
      ...
      // Act & Assert
      await expect(inativarUsuarioUseCase.execute(usuarioId)).rejects.toThrow(
    ...
    it('must deactivate the user and return code 204', async () => {
      // Arrange
      const usuarioId = 2;
      ...
      await inativarUsuarioUseCase.execute(usuarioId);
